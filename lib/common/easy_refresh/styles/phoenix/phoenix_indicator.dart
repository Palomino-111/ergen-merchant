/// 从3.4.0版本源码phoenix_indicator.dart中复制出来的
import 'package:easy_refresh/easy_refresh.dart';
import 'package:flutter/material.dart';
import 'package:zheergen_merchant_end/common/dimensions.dart';
import 'dart:math' as math;
import '../../painter/paths_painter.dart';
...
class PhoenixIndicator extends StatefulWidget {
  ...
  @override
  State<PhoenixIndicator> createState() => _PhoenixIndicatorState();
}

class _PhoenixIndicatorState extends State<PhoenixIndicator>
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // _offset相当于footer的高度，这个是由父Widget滑动时的距离决定的，
        // 通过控制控制这个变量来实现footer的padding 效果，
        // 但是会导致footer的高度比预期高度矮对应的padding值，先暂时就这样吧，后续再看看怎么调整。
        // 这个方案还有一个问题就是在当_offset达到_actualTriggerOffset时会有一个padding距离的动画跳变，
        // todo 这个解决方案就是要把_actualTriggerOffset用到的地方也减去padding距离应该就行了，后面再处理。
        // 后续可以由外部传入参数来控制padding大小，现在先写死[Dimensions.padding16]，
        // 还可以通过transfer的方式实现上下左右的padding
        final height =
            (_offset - Dimensions.padding16).clamp(0.0, double.infinity);
        return Stack(
          alignment: Alignment.center,
          children: [
            // 背景色，背景色高度决定了整个footer的高度，通过控制背景色高度来实现footer的上下padding的效果，
            // 但是会导致footer的高度比预期高度矮对应的padding值，先暂时就这样吧，后续再看看怎么调整。
            Container(
