...
#   powershell -ExecutionPolicy Bypass -File scripts/repro-login.ps1
#
# 可选：
...
# ---- anon key 缺省时从仓库里读（不在这里重复写死密钥）----
# 重构后 key 在 lib/common/env.dart 的 SUPABASE_ANON_KEY defaultValue 里；
# 旧版本在 lib/main.dart 的 anonKey: 字面量里，两个都试。
if (-not $AnonKey) {
  foreach ($rel in @('..\lib\common\env.dart', '..\lib\main.dart')) {
    $p = Join-Path $PSScriptRoot $rel
    if (-not (Test-Path $p)) { continue }
    $src = Get-Content $p -Raw
    $m = [regex]::Match($src, '(eyJ[A-Za-z0-9_\-\.]{60,})')
    if (-not $m.Success) { $m = [regex]::Match($src, "anonKey:\s*'([^']+)'") }
    if ($m.Success) { $AnonKey = $m.Groups[1].Value; break }
  }
  if (-not $AnonKey) { throw '没能从 env.dart / main.dart 解析出 anonKey，请用 $env:SB_ANON 传入' }
}
